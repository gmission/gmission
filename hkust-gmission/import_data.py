...
def clear_db():
    # map(db.session.delete, LocationMapping.query.all())
    map(db.session.delete, IndoorRectangle.query.all())
...
def import_crabwords():
    # http://minganci.sourceforge.net/2.html
    r = csv.reader(file(os.path.join(DATA_PATH, 'crabwords.csv')))
    for row in r:
        word, = row
        print word
        w = get_or_create(SensitiveWord, word=word)
    pass
